        ...
        <nav className="sidebar-nav">
          <SignedIn>
            ...
            {navLinks.slice(0, 6).map( nav => {
              ...
              return (
                <li key={nav.route} className={`sidebar-nav_element group ${isActive ? 'bg-purple-700 text-white hover:text-gray-700' :'text-gray-700'} `}>
                  <Link href={nav.route} className='sidebar-link'>
                    <Image src={nav.icon} alt={nav.label} width={24} height={24} className={`${isActive && 'brightness-200 text-gray-700'}`}/>
                    {nav.label}
                  </Link>
                </li>
              )
